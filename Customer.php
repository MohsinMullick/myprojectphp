...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //$payment_method = htmlspecialchars(trim($_POST['payment_methods'] ?? ''));  // Corrected here
    $road_no = htmlspecialchars(trim($_POST['road_number'] ?? '')); // Corrected here
    $house_no = htmlspecialchars(trim($_POST['house_number'] ?? '')); // Corrected here
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        $stmt->bind_param(
            ...
            $registration_date, $postal_code, $payment_method, $road_no, $house_no, $location
        );
